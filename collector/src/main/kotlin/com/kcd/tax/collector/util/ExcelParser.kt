package com.kcd.tax.collector.util
...
class ExcelParser {
    ...
    private fun generateSampleData(businessNumber: String): List<Transaction> {
        ...
        // 매출 데이터 생성
        repeat(SAMPLE_SALES_COUNT) {
            transactions.add(
                Transaction(
                    businessNumber = businessNumber,
                    type = TransactionType.SALES,
                    amount = BigDecimal(random.nextInt(1_000_000, 5_000_000)),
                    counterpartyName = "${CollectionConstants.COUNTERPARTY_PREFIX_SALES}${it + 1}",
                    transactionDate = LocalDate.now().minusDays(it.toLong())
                )
            )
        }

        // 매입 데이터 생성
        repeat(SAMPLE_PURCHASE_COUNT) {
            transactions.add(
                Transaction(
                    businessNumber = businessNumber,
                    type = TransactionType.PURCHASE,
                    amount = BigDecimal(random.nextInt(500_000, 2_000_000)),
                    counterpartyName = "${CollectionConstants.COUNTERPARTY_PREFIX_PURCHASE}${it + 1}",
                    transactionDate = LocalDate.now().minusDays(it.toLong())
                )
            )
        }
    ...
    fun parseExcelFile(filePath: String, businessNumber: String): List<Transaction> {
        ...
        WorkbookFactory.create(file).use {
            // 매출 시트 파싱
            it.getSheet(CollectionConstants.SHEET_NAME_SALES)?.let {
                logger.info("${CollectionConstants.SHEET_NAME_SALES} 시트 파싱 중...")
                transactions.addAll(parseSheet(it, businessNumber, TransactionType.SALES))
            }

            // 매입 시트 파싱
            it.getSheet(CollectionConstants.SHEET_NAME_PURCHASE)?.let {
                logger.info("${CollectionConstants.SHEET_NAME_PURCHASE} 시트 파싱 중...")
                transactions.addAll(parseSheet(it, businessNumber, TransactionType.PURCHASE))
            }
        }
